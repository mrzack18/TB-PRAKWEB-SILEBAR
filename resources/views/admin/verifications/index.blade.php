...
@section('title', 'Verifikasi Barang - SILEBAR')
@section('header-title', 'Verifikasi Barang')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="border rounded-lg p-4">
                <div class="flex justify-between items-start">
                    <h3 class="font-semibold text-gray-800">{{ $auction->title }}</h3>
                    <span class="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded">Pending</span>
                </div>
                
                <p class="text-sm text-gray-600 mt-2">{{ Str::limit($auction->description, 100) }}</p>
                
                <div class="mt-4">
                    <p class="text-xs text-gray-500">Oleh: {{ $auction->seller->name }}</p>
                    <p class="text-xs text-gray-500">Kategori: {{ $auction->category->name }}</p>
                    <p class="text-sm font-semibold mt-1">Rp {{ number_format($auction->starting_price, 0, ',', '.') }}</p>
                </div>
                
                <div class="mt-4 flex space-x-2">
                    <form action="{{ route('admin.verifications.approve', $auction) }}" method="POST" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700">Setujui</button>
                    </form>
                    
                    <button onclick="openRejectModal({{ $auction->id }})" class="px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700">Tolak</button>
                </div>
            </div>
            @endforeach
        </div>
