    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-primary rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
            <div class="relative z-10">
                ...
                <a href="{{ route('auctions.index') }}" class="inline-block bg-white text-primary font-bold px-6 py-2.5 rounded-lg shadow-sm hover:bg-gray-50 transition-colors">
                    Mulai Belanja
                </a>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-start p-4 bg-background rounded-xl border border-gray-100 dark:border-gray-700 transition-colors duration-300">
                    <div class="h-12 w-12 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 flex-shrink-0 overflow-hidden">
                         @if($bid->auctionItem->images->first())
                            <img src="{{ asset('storage/' . $bid->auctionItem->images->first()->image_path) }}" class="h-full w-full object-cover">
                        @endif
                    </div>
                    <div class="ml-4 flex-1">
                        <p class="text-sm font-medium text-text-main transition-colors">Anda menawar pada <span class="font-bold text-primary">{{ $bid->auctionItem->title }}</span></p>
                        <p class="text-xs text-text-muted mt-1 transition-colors">{{ $bid->created_at->diffForHumans() }}</p>
                    </div>
                    <div class="text-right">
                        <span class="block text-primary font-bold">Rp {{ number_format($bid->bid_amount, 0, ',', '.') }}</span>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="bg-surface p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 text-center transition-colors duration-300">
                <h4 class="text-2xl font-bold text-primary">{{ $stats['bids_count'] ?? 0 }}</h4>
                <p class="text-xs text-text-muted uppercase tracking-wide mt-1 transition-colors">Total Tawaran</p>
            </div>
            <div class="bg-surface p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 text-center transition-colors duration-300">
                <h4 class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $stats['won_auctions'] ?? 0 }}</h4>
        ...
        <div class="bg-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors duration-300">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('buyer.auctions.followed') }}" class="text-xs text-primary hover:underline">Lihat Semua</a>
            </div>
            <div class="space-y-3">
                 @forelse($followedAuctions ?? [] as $auction)
                 <a href="{{ route('auctions.show', $auction) }}" class="flex items-center group">
                    ...
                    <div class="ml-3 overflow-hidden">
                        <p class="text-sm font-medium text-text-main group-hover:text-primary truncate transition-colors">{{ $auction->title }}</p>
                        <p class="text-xs text-red-500">{{ $auction->time_remaining }} lagi</p>
                    </div>
                 </a>
                 @empty
