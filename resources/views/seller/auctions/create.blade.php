...
@section('title', 'Tambah Barang - SILEBAR')
@section('header-title', 'Tambah Barang Lelang')
        ...
        <form method="POST" action="{{ route('seller.auctions.store') }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                </div>
                
                <div>
                    <label for="description" class="block text-sm font-medium text-text-muted transition-colors">Deskripsi Barang</label>
                    <textarea name="description" id="description" rows="4" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="starting_price" id="starting_price" value="{{ old('starting_price') }}" required min="0" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="duration_value" id="duration_value" value="{{ old('duration_value', 1) }}" required min="1" max="30" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                    ...
                    <div>
                        ...
                        <select name="duration_unit" id="duration_unit" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                            <option value="minutes" {{ old('duration_unit') == 'minutes' ? 'selected' : '' }}>Menit</option>
                            <option value="hours" {{ old('duration_unit') == 'hours' ? 'selected' : '' }}>Jam</option>
                            <option value="days" {{ old('duration_unit') == 'days' ? 'selected' : '' }} selected>Hari</option>
                        </select>
