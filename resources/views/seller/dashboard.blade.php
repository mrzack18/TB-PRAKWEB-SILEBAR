    ...
    <div class="bg-gradient-to-r from-primary to-blue-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-lg shadow-blue-200 dark:shadow-none">
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="mt-6 md:mt-0">
                <a href="{{ route('seller.auctions.create') }}" class="inline-flex items-center px-6 py-3 bg-white text-primary font-bold rounded-xl shadow-lg hover:shadow-xl hover:bg-gray-50 transform hover:-translate-y-1 transition-all">
    ...
    <div class="bg-surface p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:border-primary/30 transition-colors duration-300">
        <div class="flex justify-between items-start">
            <div>
                ...
                <h3 class="text-3xl font-bold text-text-main mt-1 transition-colors">{{ $stats['total_auctions'] ?? 0 }}</h3>
            </div>
    ...
    <div class="bg-surface p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:border-green-500/30 transition-colors duration-300">
        <div class="flex justify-between items-start">
            <div>
                ...
                <h3 class="text-3xl font-bold text-text-main mt-1 transition-colors">{{ $stats['active_auctions'] ?? 0 }}</h3>
            </div>
    ...
    <div class="bg-surface p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:border-accent/30 transition-colors duration-300">
        <div class="flex justify-between items-start">
            <div>
                ...
                <h3 class="text-2xl font-bold text-text-main mt-1 transition-colors">Rp {{ number_format($stats['estimated_revenue'] ?? 0, 0, ',', '.') }}</h3>
            </div>
...
<div class="bg-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors duration-300">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('seller.auctions.index') }}" class="text-sm text-primary hover:text-blue-700 font-medium">Lihat Semua</a>
    </div>
                ...
                <tr class="group hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <td class="px-4 py-4">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-text-main transition-colors">{{ Str::limit($auction->title, 30) }}</p>
                                <p class="text-xs text-text-muted transition-colors">{{ $auction->created_at->format('d M Y') }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-4 py-4 text-sm font-bold text-text-main transition-colors">Rp {{ number_format($auction->current_price, 0, ',', '.') }}</td>
                    <td class="px-4 py-4">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $auction->status == 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400' }}">
                            {{ ucfirst($auction->status) }}
                        </span>
                    </td>
                    <td class="px-4 py-4 text-sm text-text-muted transition-colors">{{ $auction->bids_count ?? 0 }}</td>
                    <td class="px-4 py-4 text-right">
                        <a href="{{ route('seller.auctions.edit', $auction) }}" class="text-primary hover:text-blue-800 text-sm font-medium">Edit</a>
                    </td>
                </tr>
                @empty
                <tr>
