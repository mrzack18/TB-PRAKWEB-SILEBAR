...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="bg-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
        <div class="flex flex-col md:flex-row gap-6">
            <div class="md:w-1/3">
                <?php if($transaction->auctionItem->images->first()): ?>
                    <img src="<?php echo e(asset('storage/' . $transaction->auctionItem->images->first()->image_path)); ?>" 
                         alt="<?php echo e($transaction->auctionItem->title); ?>" 
            ...
            <div class="md:w-2/3">
                <h2 class="text-2xl font-bold text-text-main"><?php echo e($transaction->auctionItem->title); ?></h2>
                <p class="text-text-muted mt-2"><?php echo e($transaction->auctionItem->description); ?></p>
                
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-text-muted">Harga Akhir</p>
                        <p class="text-lg font-bold text-primary">Rp <?php echo e(number_format($transaction->final_price, 0, ',', '.')); ?></p>
                    </div>
                    <div>
                        <p class="text-sm text-text-muted">Komisi (10%)</p>
                        <p class="text-lg font-bold text-red-600">Rp <?php echo e(number_format($transaction->commission_amount, 0, ',', '.')); ?></p>
                    </div>
                    <div>
                        <p class="text-sm text-text-muted">Untuk Penjual (90%)</p>
                        <p class="text-lg font-bold text-green-600">Rp <?php echo e(number_format($transaction->seller_amount, 0, ',', '.')); ?></p>
                    </div>
                    <div>
                        <p class="text-sm text-text-muted">Tanggal Pembayaran</p>
                        <p class="text-lg font-bold text-text-main"><?php echo e($transaction->created_at->format('d M Y H:i')); ?></p>
                    </div>
                    <div>
                        <p class="text-sm text-text-muted">Status Pembayaran</p>
                        <span class="inline-block px-3 py-1 text-sm font-semibold rounded-full 
                            <?php if($transaction->payment_status == 'verified'): ?> bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                            <?php elseif($transaction->payment_status == 'pending_verification'): ?> bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400
                            <?php elseif($transaction->payment_status == 'rejected'): ?> bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400
                            <?php else: ?> bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400 <?php endif; ?>">
                    ...
                    <div>
                        ...
                        <span class="inline-block px-3 py-1 text-sm font-semibold rounded-full 
                            <?php if($transaction->shipping_status == 'delivered'): ?> bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                            <?php elseif($transaction->shipping_status == 'shipped'): ?> bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400
                            <?php elseif($transaction->shipping_status == 'waiting_shipment'): ?> bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400
                            <?php else: ?> bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400 <?php endif; ?>">
    ...
    <?php if($transaction->payment_status == 'pending' && auth()->id() == $transaction->winner_id): ?>
    <div class="bg-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
        <h3 class="text-xl font-bold text-text-main mb-4">Proses Pembayaran</h3>
        
        <form action="<?php echo e(route('payments.process', $transaction)); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
    ...
    <div class="bg-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div class="flex items-center">
            <div class="h-16 w-16 rounded-lg bg-gray-200 dark:bg-gray-700 overflow-hidden">
                <img src="<?php echo e(asset('storage/' . $transaction->payment_proof)); ?>" class="h-full w-full object-cover">
            </div>
            <div class="ml-4">
                <p class="text-sm font-medium text-text-main">Bukti Pembayaran</p>
                <p class="text-xs text-text-muted"><?php echo e($transaction->payment_method ? ucfirst(str_replace('_', ' ', $transaction->payment_method)) : 'Metode Tidak Ditentukan'); ?></p>
            </div>
        </div>
    </div>
    <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\LENOVO\Desktop\TB\TB-PRAKWEB-SILEBAR\resources\views/payments/show.blade.php ENDPATH**/ ?>