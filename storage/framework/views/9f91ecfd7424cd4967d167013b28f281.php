...
<?php $__env->startSection('title', 'Tambah Barang - SILEBAR'); ?>
<?php $__env->startSection('header-title', 'Tambah Barang Lelang'); ?>
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-surface shadow rounded-lg p-6 border border-gray-100 dark:border-gray-700 transition-colors duration-300">
        <h2 class="text-xl font-semibold text-text-main mb-6 transition-colors">Tambah Barang Lelang Baru</h2>
        
        <form method="POST" action="<?php echo e(route('seller.auctions.store')); ?>" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
            
            <div class="space-y-4">
                <div>
                    <label for="title" class="block text-sm font-medium text-text-muted transition-colors">Judul Barang</label>
                    <input type="text" name="title" id="title" value="<?php echo e(old('title')); ?>" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                </div>
                
                <div>
                    <label for="description" class="block text-sm font-medium text-text-muted transition-colors">Deskripsi Barang</label>
                    <textarea name="description" id="description" rows="4" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors"><?php echo e(old('description')); ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="starting_price" id="starting_price" value="<?php echo e(old('starting_price')); ?>" required min="0" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                    </div>
                </div>

                <div>
                    <label for="images" class="block text-sm font-medium text-text-muted transition-colors">Gambar Barang</label>
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md transition-colors hover:border-gray-400 dark:hover:border-gray-500">
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-text-muted transition-colors">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="duration_value" id="duration_value" value="<?php echo e(old('duration_value', 1)); ?>" required min="1" max="30" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                    ...
                    <div>
                        ...
                        <select name="duration_unit" id="duration_unit" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-text-main focus:ring-primary focus:border-primary transition-colors">
                            <option value="minutes" <?php echo e(old('duration_unit') == 'minutes' ? 'selected' : ''); ?>>Menit</option>
                            <option value="hours" <?php echo e(old('duration_unit') == 'hours' ? 'selected' : ''); ?>>Jam</option>
                            <option value="days" <?php echo e(old('duration_unit') == 'days' ? 'selected' : ''); ?> selected>Hari</option>
                        </select>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-700 transition-colors">Simpan Barang</button>
            </div>
        </form>
    </div>
</div>
    ...
    if (input.files) {
        ...
        filesArray.forEach((file, index) => {
            ...
            reader.onload = function(e) {
                ...
                img.className = 'w-full h-32 object-cover rounded border border-gray-300 dark:border-gray-600';
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\LENOVO\Desktop\TB\TB-PRAKWEB-SILEBAR\resources\views/seller/auctions/create.blade.php ENDPATH**/ ?>